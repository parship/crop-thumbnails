...
add_action( 'after_setup_theme', function () {
	...
	add_filter( 'crop_thumbnails_should_delete_old_file',
		[ CptSaveThumbnail::class, 'filter_shouldDeleteOldFile' ],
		10,
		4 );
}, 10 );
...
class CptSaveThumbnail {
	...
	public static function saveThumbnail() {
		...
		try {
			...
			$imageMetadata = wp_get_attachment_metadata( $input->sourceImageId,
				true );//get the attachement metadata of the post
			...
			foreach ( $input->activeImageSizes as $activeImageSize ) {
				...
				$currentFilePath = self::generateFilename( $sourceImgPath,
					$imageMetadata,
					$croppedSize['width'],
					$croppedSize['height'],
					$activeImageSize->crop );
				...
				$resultWpCropImage = apply_filters( 'crop_thumbnails_do_crop',
					null,
					$input,
					$croppedSize,
					$temporaryCopyFile,
					$currentFilePath );
				do_action( 'crop_thumbnails_after_crop',
					$input,
					$croppedSize,
					$temporaryCopyFile,
					$currentFilePath,
				...
				if ( empty( $resultWpCropImage ) || is_wp_error( $resultWpCropImage ) ) {
					$_processing_error[ $activeImageSize->name ][] = sprintf( __( "Can't generate filesize '%s'.",
						'crop-thumbnails' ),
						$activeImageSize->name );
					$_error                                        = true;
				} else {
					...
					if ( ! @copy( $resultWpCropImage, $currentFilePath ) ) {
						$_processing_error[ $activeImageSize->name ][] = __( "Can't copy temporary file to media library.",
							'crop-thumbnails' );
						$_error                                        = true;
					}
					if ( ! @unlink( $resultWpCropImage ) ) {
						$_processing_error[ $activeImageSize->name ][] = __( "Can't delete temporary file.",
							'crop-thumbnails' );
						$_error                                        = true;
					}
				}
				...
				if ( ! $_error ) {
					...
					$imageMetadata = self::createNewMetadata( $imageMetadata,
						$activeImageSize->name,
						$currentFilePathInfo,
						$croppedSize['width'],
						$croppedSize['height'],
			...
			$imageMetadata = apply_filters( 'crop_thumbnails_before_update_metadata',
				$imageMetadata,
				$input->sourceImageId );
	 ...
	 * @return bool
	 */
	public static function filter_shouldDeleteOldFile(
		$baseResult,
		$oldSizeMetadata,
		$activeImageSize,
		$activeImageFilePath
	...
	public static function filter_doWpCrop( $baseResult, $input, $croppedSize, $temporaryCopyFile, $currentFilePath ) {
		return wp_crop_image(                                // * @return string|WP_Error|false New filepath on success, WP_Error or false on failure.
			...
			$input->selection->x2 - $input->selection->x,
			// * @param int $src_w The width to crop.
			$input->selection->y2 - $input->selection->y,
	 ...
	 */
	protected static function createNewMetadata(
		$imageMetadata,
		$imageSizeName,
		$currentFilePathInfo,
		$croppedWidth,
		...
		do_action( 'crop_thumbnails_after_save_new_thumb',
			$fullFilePath,
			$imageSizeName,
	...
	protected static function getValidatedInput() {
		...
		if ( ! check_ajax_referer( $GLOBALS['CROP_THUMBNAILS_HELPER']->getNonceBase(), '_ajax_nonce', false ) ) {
			throw new Exception( __( "ERROR: Security Check failed (maybe a timeout - please try again).",
				'crop-thumbnails' ), 1 );
		}
		...
		if ( $input->selection->x < 0 || $input->selection->y < 0 ) {
			throw new Exception( __( 'Cropping to these dimensions on this image is not possible.', 'crop-thumbnails' ),
				1 );
		}
