...
class CropThumbnailsSettingsScreen {
	...
	public function optionsPageStyle() {
		if ( ! empty( $_REQUEST['page'] ) && $_REQUEST['page'] === 'page-cpt' ) {
			wp_enqueue_style( 'crop-thumbnails-options-style',
				plugins_url( 'app/css/app.css', __DIR__ ),
				[],
				CROP_THUMBNAILS_VERSION );
			wp_enqueue_script( 'vue', plugins_url( 'app/js/chunk-vendors.js', __DIR__ ), [], CROP_THUMBNAILS_VERSION );
			wp_enqueue_script( 'crop-thumbnails-options-js',
				plugins_url( 'app/js/app.js', __DIR__ ),
				[ 'vue' ],
				CROP_THUMBNAILS_VERSION );
		}
	}
	...
	public function addSettingsLinkToPluginPage( $links, $file ) {
		if ( $file === 'crop-thumbnails/crop-thumbnails.php' ) {
			$settings_link = '<a href="options-general.php?page=page-cpt" title="">' . esc_html__( 'Settings',
					'crop-thumbnails' ) . '</a>';
	...
	public function addOptionsPage() {
		add_options_page( esc_html__( 'Crop Post Thumbnail Page', 'crop-thumbnails' ),
			'Crop-Thumbnails',
			'manage_options',
			'page-cpt',
			[ $this, 'optionsPage' ] );
		...
		<div class="wrap cropThumbnailSettings">
			...
			<form action="options.php" method="post">
				...
				<div class="<?php echo self::$cssPrefix ?>submit">
					<input name="Submit" type="submit" value="<?php esc_attr_e( 'Save Changes', 'crop-thumbnails' ); ?>"
			...
			<div class="<?php echo self::$cssPrefix; ?>paypal">
				...
				<p><?php esc_html_e( 'You can support the plugin author (and let him know you love this plugin) by donating via Paypal. Thanks a lot!',
						'crop-thumbnails' ); ?></p>
	...
	public function settingsInitialisation() {
		register_setting( self::$uniqeSettingsId,
			$GLOBALS['CROP_THUMBNAILS_HELPER']->getOptionsKey(),
			[ $this, 'validateSettings' ] );

		$_sectionID = 'choose_sizes_section';
		add_settings_section( $_sectionID,
			esc_html__( 'Sizes and Post Types', 'crop-thumbnails' ),
			[ $this, 'sectionDescriptionChooseSizes' ],
			'page1' );

		$_sectionID = 'userPermission';
		add_settings_section( $_sectionID,
			esc_html__( 'User Permission', 'crop-thumbnails' ),
			[ $this, 'emptySectionDescription' ],
			'page1' );
		$_tmpID = 'user_permission_only_on_edit_files';
		add_settings_field( $_tmpID,
			esc_html__( 'When active, only users who are able to edit files can crop thumbnails. Otherwise (default), any user who can upload files can also crop thumbnails.',
				'crop-thumbnails' ),
			[ $this, 'callback_' . $_tmpID ],
			'page1',
			$_sectionID,
			[ 'label_for' => self::$cssPrefix . $_tmpID ] );

		$_sectionID = 'quick_test';
		add_settings_section( $_sectionID,
			esc_html__( 'Plugin Test', 'crop-thumbnails' ),
			[ $this, 'sectionDescriptionTest' ],
			'page1' );

		$_sectionID = 'developer';
		add_settings_section( $_sectionID,
			esc_html__( 'Developer Settings', 'crop-thumbnails' ),
			[ $this, 'emptySectionDescription' ],
			'page1' );
		$_tmpID = 'debug_js';
		add_settings_field( $_tmpID,
			esc_html__( 'Enable JS-Debug.', 'crop-thumbnails' ),
			[ $this, 'callback_' . $_tmpID ],
			'page1',
			$_sectionID,
			[ 'label_for' => self::$cssPrefix . $_tmpID ] );
		$_tmpID = 'debug_data';
		add_settings_field( $_tmpID,
			esc_html__( 'Enable Data-Debug.', 'crop-thumbnails' ),
			[ $this, 'callback_' . $_tmpID ],
			'page1',
			$_sectionID,
			[ 'label_for' => self::$cssPrefix . $_tmpID ] );
	...
	protected function vueSettingsScreen() {
		$settings = [
			...
			'lang'        => [
				'choose_image_sizes' => esc_js( __( 'Choose the image sizes you do not want to show, if the user uses the button below the featured image box.',
					'crop-thumbnails' ) ),
				'hide_on_post_type'  => esc_js( __( 'Hide Crop-Thumbnails button below the featured image?',
					'crop-thumbnails' ) ),
			],
		];
	...
	public function sectionDescriptionChooseSizes() { ?>
		<p>
			<?php esc_html_e( 'Crop-Thumbnails is designed to make cropping images easy. For some post types, not all crop sizes are needed, but the plugin will automatically create all the crop sizes. Here you can select which crop sizes are available in the cropping interface for each post type..',
				'crop-thumbnails' ) ?>
			<br/><strong><?php esc_html_e( 'Crop-Thumbnails will only show cropped images. Sizes with no crop will always be hidden.',
					'crop-thumbnails' ); ?></strong>
		</p>
		<?php
	...
	public function callback_user_permission_only_on_edit_files() {
		...
		echo '<input name="' . $GLOBALS['CROP_THUMBNAILS_HELPER']->getOptionsKey() . '[' . $_id . ']" id="' . self::$cssPrefix . $_id . '" type="checkbox" value="1" ' . checked( 1,
				$options[ $_id ],
				false ) . ' />';
		?>
		<div class="<?php echo self::$cssPrefix ?>submit">
	...
	public function ajax_callback_admin_quicktest() {
		...
		try {
			...
			$attachmentId            = media_handle_upload( 'cpt_quicktest',
				0,
				[],
				[ 'test_form' => false, 'action' => 'test' ] );
